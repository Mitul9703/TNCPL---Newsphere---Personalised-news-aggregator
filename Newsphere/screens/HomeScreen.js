...
export default function HomeScreen({ navigation }) {
  ...
  const fetchArticles = async () => {
    ...
    try {
      const response = await fetch(`${NgrokURL}/recommendations`, {
        method: "POST",
        headers: {
      ...
      if (json.recommendations) {
        ...
        setArticles(json.recommendations); // Set the articles in state
      } else {
        console.error("Failed to load recommendations:", json.message);
      }
    } catch (error) {
    ...
    return (
      <View style={styles.centeredContent}>
        ...
        <Text style={{ fontSize: 14, fontWeight: "400", padding: 10 }}>
          Fetching the right articles for you...
        </Text>
      </View>
    );
  }
    ...
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          ...
          >
            Here's your latest news roundup.
          </Text>
        }
        refreshControl={
          <RefreshControl
