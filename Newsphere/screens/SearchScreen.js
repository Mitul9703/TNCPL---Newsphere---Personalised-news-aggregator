  ...
  const fetchArticles = async () => {
    ...
    try {
      const response = await fetch(
        `${NgrokURL}/search?query=${encodeURIComponent(query)}`
      );
      const json = await response.json();
      if (response.ok) {
        setArticles(json.search_results);
      } else {
        console.error("Failed to fetch articles:", json.message);
        setArticles([]); // Clear articles on failure
      }
    } catch (error) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <TextInput
        ...
        onSubmitEditing={fetchArticles} // Fetch articles when the user submits the query
      />
      {isLoading ? (
